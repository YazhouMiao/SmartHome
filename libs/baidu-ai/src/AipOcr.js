...
class AipOcr extends BaseClient {
    ...
    idcard(image, isFront, options) {
        ...
        let param = {
            image: image,
            id_card_side: side,
            endpoint: PATH_OCR_IDCARD
        };
        let promise = this.registTask(this.commonImpl, objectTools.merge(param, options));
        return promise;
    }
    generalUrl(url, options) {
        let param = {url: url, endpoint: PATH_OCR_GENERAL};
        let promise = this.registTask(this.commonImpl, objectTools.merge(param, options));
        return promise;
    }
    general(image, options) {
        let param = {image: image, endpoint: PATH_OCR_GENERAL};
        let promise = this.registTask(this.commonImpl, objectTools.merge(param, options));
        return promise;
    }
    accurate(image, options) {
        let param = {image: image, endpoint: PATH_OCR_ACCURATE};
        let promise = this.registTask(this.commonImpl, objectTools.merge(param, options));
        return promise;
    }
    generalBasicUrl(url, options) {
        let param = {url: url, endpoint: PATH_OCR_GENERAL_BASIC};
        let promise = this.registTask(this.commonImpl, objectTools.merge(param, options));
        return promise;
    }
    generalBasic(image, options) {
        let param = {image: image, endpoint: PATH_OCR_GENERAL_BASIC};
        let promise = this.registTask(this.commonImpl, objectTools.merge(param, options));
        return promise;
    }
    accurateBasic(image, options) {
        let param = {image: image, endpoint: PATH_OCR_ACCURATE_BASIC};
        let promise = this.registTask(this.commonImpl, objectTools.merge(param, options));
        return promise;
    }
    generalEnhanceUrl(url, options) {
        let param = {url: url, endpoint: PATH_OCR_GENERAL_ENHANCE};
        let promise = this.registTask(this.commonImpl, objectTools.merge(param, options));
        return promise;
    }
    generalEnhance(image, options) {
        let param = {image: image, endpoint: PATH_OCR_GENERAL_ENHANCE};
        let promise = this.registTask(this.commonImpl, objectTools.merge(param, options));
        return promise;
    }
    webImageUrl(url, options) {
        let param = {url: url, endpoint: PATH_OCR_WEBIMAGE};
        let promise = this.registTask(this.commonImpl, objectTools.merge(param, options));
        return promise;
    }
    webImage(image, options) {
        let param = {image: image, endpoint: PATH_OCR_WEBIMAGE};
        let promise = this.registTask(this.commonImpl, objectTools.merge(param, options));
        return promise;
    }
    drivingLicense(image, options) {
        let param = {image: image, endpoint: PATH_OCR_DRIVINGLICENSE};
        let promise = this.registTask(this.commonImpl, objectTools.merge(param, options));
        return promise;
    }
    licensePlate(image, options) {
        let param = {image: image, endpoint: PATH_OCR_LICENSEPLATE};
        let promise = this.registTask(this.commonImpl, objectTools.merge(param, options));
        return promise;
    }
    vehicleLicense(image, options) {
        let param = {image: image, endpoint: PATH_OCR_VEHICLELICENSE};
        let promise = this.registTask(this.commonImpl, objectTools.merge(param, options));
        return promise;
    }
